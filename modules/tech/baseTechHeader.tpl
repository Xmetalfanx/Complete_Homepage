<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <!--  REMEMBER TO EDIT THE PAGE BY PAGE TITLES TO NOT INCLUDE THE FIRST PART of the tile -->
    <title> Xmetalfanx's Homepage - <?php echo $the_title; ?> </title>

    <meta name="Description" content="Xmetafanx's Homepage is a site that can help you with Computer Problem, File Sharing News, Computer FAQ's, Security Information, Antivirus and Spyware Removal, and much much more..">

   	<!-- Various Meta Tags -->
    <meta name="robots" content="index,follow,noodp">
    <meta name="googlebot" content="index,follow">

    <link rel="alternate" href="xml/site_updates.xml" type="application/rss+xml" title="Xmetalfanx Site Updates and Information">

    <link rel="alternate" href="xml/program_updates.xml" type="application/rss+xml" title="Xmetalfanx Recently Added Programs">

    <link rel="index" href="/index.php">
    <link rel="home" href="/index.php">

    <!-- jQuery library -->
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js"></script>

    <!-- Link to External Style Sheet -->
    <link href="/techHQ/stylesheets/layout.css" rel="stylesheet" type="text/css">
    <link href="/techHQ/stylesheets/general.css" rel="stylesheet" type="text/css" >
    <link href="/techHQ/stylesheets/startPageBox.css" rel="stylesheet" type="text/css">

    <!-- Latest compiled and minified CSS -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" >

 </head>
<!-- what USE TO BE in a seperate Head module is what is above this line -->
<body>

<div id="top">

		<div id="bannerText">Xmetal's TechHQ </div>
		<input type="button" value="Menu" id="navButton" />

		<!-- main site navigation -->
		<nav>
			  <a href="/techHQ/software/softwareIndex.php" accesskey="1">Software </a>
			  <a href="/techHQ/techcenter/techCenterIndex.php" accesskey="2">Tech Center ( Tips/Tutorials/Guides )</a>
			  <a href="/techHQ/browsers/browserIndex.php" accesskey="3">Web Browser </a>
			  <a href="/techHQ/security/securityIndex.php" accesskey="4">Security</a>
			  <!-- <a href="/techHQ/guides/guidesIndex.php" accesskey="5">PDF Guides</a> -->
		</nav>
</div>

<!-- no clue if these lines below here are a good idea -->
		<div id="mainBody">
