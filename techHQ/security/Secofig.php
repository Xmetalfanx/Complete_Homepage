...
?>
    ...
    <p class="center">
      <a href="https://seconfig.sytes.net/" target="_blank">https://seconfig.sytes.net/</a>
    </p>
    <p class="quoted">This is a tool for configuring mostly hidden Windows 2000/XP/2003 (and probably up) settings. With this tool it is easy to adjust Windows to relatively good network security level.<br />  Settings are divided in three sections:
    </p>
    <p class="quoted"><strong>Restrict Microsoft Networks</strong><br />  This part mostly disable use of TCP/IP protocol as a transport for network access to this computer in the way it is done in Microsoft Networks. TCP/IP is the protocol suite used by the internet and most network attacks (by worms, hackers etc.) to Windows systems are using Microsoft Networks like access via internet, thus these options provide a way to stop most attacks to Windows operating system from the internet.
    </p>
    <p class="quoted"><strong>Services settings</strong><br />  Here can be changed settings for mainly insecure services, usually used in LANs or not used at all.
    </p>
    <p class="quoted"><strong>TCP/IP settings</strong><br />  Here are some settings to disable insecure and mostly unused/unneeded features of Windows TCP/IP stack. It is also possible to change starting ephemeral port here.<br />
    </p>
    <p class="quoted"><strong>Detailed descriptions</strong>
    </p>
    <p class="quoted"><strong>Disable NetBIOS over TCP/IP (all adapters)</strong><br />  This option unbinds NetBT (NetBIOS over TCP/IP) from all network adapters (closing ports 137-139 on all currently installed network interfaces). <br />  This is very similar to selecting &quot;Disable NetBIOS over TCP/IP&quot; (in Advanced... - WINS tab) TCP/IP option for all network adapters.<br />  Note: Unlike manual selecting, this also affects hidden adapters. From the other hand - hidden adapters usually have ability to set this option on per connection basis.
    </p>
    ...
    <p class="quoted"><strong>Do not start IPSEC Services service automatically</strong><br />  This changes startup type of IPSEC Services service to manual. This setting should be enabled only on systems NOT using IPSec (most systems). IPSEC Services service simply is not usually needed and opens some otherwise closed ports.
    </p>
    <p class="quoted"><strong>Drop all incoming IP source routed packets</strong><br />  This option tells Windows to ignore all incoming IP packets with source route options. Source routing is often used for IP address spoofing and very rarely required for normal network operations. In most cases dropping all incoming IP source routed packets is a good security practice.
    </p>
    ...
    <p class="quoted"><strong>Restore</strong><br />  Every setting changed by Seconfig XP was backed up (when changed first time). This button restores all settings to backed up values and restarts computer.
    </p>
    <p class="quoted"><strong>For MN</strong><br />  This button loads/generates settings best suited for average Microsoft Networks workstation/server.
    </p>
    <p class="quoted"><strong>For home</strong><br />  This Button loads/generates settings best suited for average standalone computer with no need for Microsoft Networks.
    </p>
