
<div class="accordion">

  <!-- the input ID and for="" have to be equal-->
  <input id="themePPA" type="checkbox">
  <label for="themePPA">Mass Theme PPAs</label>
  <div class="content">
    <a href="/linuxHQ/tic/themes/adapta.php">Adapta</a>
    <a href="/linuxHQ/tic/themes/arc.php">Arc</a>
    <a href="/linuxHQ/tic/themes/minty.php">(Linux) Mint-Y</a>
    <a href="/linuxHQ/tic/themes/numix.php">Numix</a>
  </div>

  <!-- the input ID and for="" have to be equal-->
  <input id="icons" type="checkbox">
  <label for="icons">Icons</label>
  <div class="content">
    <a href="#">Evolvere</a>
    <a href="#">Faience</a>
    <a href="#">Faenza-Cupertino</a>
    <a href="#">Vibrancy Colors</a>
    <a href="#">Vivacious</a>
  </div>

  <!-- the input ID and for="" have to be equal-->
  <input id="cursors" type="checkbox">
  <label for="cursors">Cursors</label>
  <div class="content"></div>

  <!-- the input ID and for="" have to be equal-->
  <input id="links" type="checkbox">
  <label for="links">External Links</label>
  <div class="content">
    <a href="https://www.deviantart.com/" target="_blank" aria-label="Deviant Art Site Link">DevianArt
    <i class="m-2 fa fa-4x fa-deviantart text-dark" aria-hidden="true"></i>
    </a>
  </div>
</div>
